...
%% ========== CREATE SINGLE FIGURE WITH 4 PANELS ==========
fig = figure('Name','Figure 4: Spiking Network Dynamics','Position',[50 50 1400 1000], 'Color', 'w');

%% Panel A: Q vs Noise (gamma comparison, MOD vs OLD)
...
title(sprintf('A: Q vs Noise (\\alpha=%.3f, \\beta=%.1f fixed) [MOD vs OLD]', alpha_val, beta_fixed_val), 'Interpreter', 'tex');
...
%% Panel B: Q vs Noise (beta comparison, MOD vs OLD)
...
xlabel('Noise \sigma'); ylabel('Recall quality Q');
title(sprintf('B: Q vs Noise (\\alpha=%.3f, \\gamma=%.1f fixed) [MOD vs OLD]', alpha_val, gamma_fixed_val), 'Interpreter', 'tex');
legend('Location','southwest', 'Interpreter', 'tex'); ylim([0,1]); xlim([0,1]); hold off;
...
title('C: median \sigma_0  \Delta Q', 'Interpreter','tex');
hold on;
...
hold off;
%% Panel D: Winners map (MOD, masked) - two side-by-side heatmaps
% Left heatmap: gamma*
axes('Position', [0.55, 0.11, 0.18, 0.35]);
imagesc(noise_levels, loading_factors, gamma_star_masked);
set(gca,'YDir','normal');
colorbar;
xlabel('Noise \sigma','Interpreter','tex', 'FontSize', 10); 
ylabel('\alpha','Interpreter','tex', 'FontSize', 10);
title('D: \gamma^* maximizing Q', 'Interpreter','tex', 'FontSize', 11);
colormap(gca, jet);

% Right heatmap: beta*
axes('Position', [0.78, 0.11, 0.18, 0.35]);
imagesc(noise_levels, loading_factors, beta_star_masked);
set(gca,'YDir','normal');
colorbar;
xlabel('Noise \sigma','Interpreter','tex', 'FontSize', 10); 
ylabel('\alpha','Interpreter','tex', 'FontSize', 10);
title('\beta^* maximizing Q', 'Interpreter','tex', 'FontSize', 11);
colormap(gca, jet);
%% ----------------- Console summaries -----------------
fprintf('\n=== Panel C summaries ===\n');
...
%% Add overall title
sgtitle('Figure 4: Spiking Network Dynamics (OLD vs MOD)', 'FontSize', 14, 'FontWeight', 'bold');

%% Save all figures
saveas(fig, '../../figures_out/main/fig04_spiking_network_dynamics.png');
saveas(fig, '../../figures_out/main/fig04_spiking_network_dynamics.fig');
